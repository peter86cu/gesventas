...
if($action == 'ajax'){
	...
	if (isset($_REQUEST['id'])){
		...
		$delete=mysqli_query($con,"delete from ordenes_de_compras_detalle where id_orden_de_compra_detalle='$id'");
	}
 ...
 if($action2=="editar"){
	...
	$query=mysqli_query($con,"select o.id_orden_de_compra_detalle as id_detalle, o.cantidad, o.importe, p.codigo,p.nombre
                             from ordenes_de_compras_detalle o inner join productos p 
                             on (o.id_producto=p.id_producto) and o.id_orden_compra=".$idOrden."  group by p.codigo");
	$items=1;
	$suma=0;
	...
	<tr>
		<td colspan='7'>
			...
			<button id="itemsBtt" type="button" class="btn btn-info btn-sm" data-toggle="modal" style ="float: left;" data-target="#myModal" <?php echo $dis ?> ><span class="glyphicon glyphicon-plus"></span> Agregar Ítem</button>
		</td>
	</tr>
	<tr>
